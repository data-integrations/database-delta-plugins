...
public class SqlServerConnection extends JdbcConnection {
  ...
  private static final String GET_MAX_TRANSACTION_LSN = "SELECT MAX(start_lsn) FROM [#db].cdc.lsn_time_mapping WHERE " +
    "tran_id <> 0x00";
  private static final String GET_NTH_TRANSACTION_LSN_FROM_BEGINNING = "SELECT MAX(start_lsn) FROM " +
    "(SELECT TOP (?) start_lsn FROM [#db].cdc.lsn_time_mapping WHERE tran_id <> 0x00 ORDER BY start_lsn) as next_lsns";
  private static final String GET_NTH_TRANSACTION_LSN_FROM_LAST = "SELECT MAX(start_lsn) FROM " +
    "(SELECT TOP (? + 1) start_lsn FROM [#db].cdc.lsn_time_mapping WHERE start_lsn >= ? AND " +
    "tran_id <> 0x00 ORDER BY start_lsn) as next_lsns";
  ...
  private static final String GET_ALL_CHANGES_FOR_TABLE = "SELECT *# FROM " +
    "[#db].cdc.[fn_cdc_get_all_changes_#](?, ?, N'all update old') order by " +
    "[__$start_lsn] ASC, [__$seqval] ASC, [__$operation] ASC";
  private final String getAllChangesForTableFirst;
   ...
   * We create a jdbcDriverClass, so that we can set the value in app source.
   */
  public static Class<? extends Driver> jdbcDriverClass;
  ...
  private static final String GET_CHANGE_TABLES = "WITH ordered_change_tables" +
    " AS (SELECT ROW_NUMBER() OVER (PARTITION BY ct.source_object_id, ct.start_lsn ORDER BY ct.create_date DESC) AS " +
    "ct_sequence," +
    " ct.*" +
  ...
  private static final String GET_NEW_CHANGE_TABLES = "SELECT * FROM [#db].cdc.change_tables WHERE " +
    "start_lsn BETWEEN ? AND ?";
  ...
  public SqlServerConnection(JdbcConfiguration config, SourceTimestampMode sourceTimestampMode,
    ...
    super(config, createConnectionFactory(multiPartitionMode), classLoaderSupplier, OPENING_QUOTING_CHARACTER,
    ...
    if (!skippedOperations.isEmpty()) {
      ...
      skippedOperations.forEach((Envelope.Operation operation) -> {
        ...
        // https://docs.microsoft.com/en-us/sql/relational-databases/system-functions/
        // cdc-fn-cdc-get-all-changes-capture-instance-transact-sql?view=sql-server-ver15#table-returned
        switch (operation) {
          case CREATE:
            skippedOps.add("2");
            break;
      ...
      getAllChangesForTableFirst = getAllChangesForTableStatement.toString();
    } else {
      getAllChangesForTableFirst = GET_ALL_CHANGES_FOR_TABLE;

    }

    getAllChangesForTable = getAllChangesForTableFirst
  ...
  public static ConnectionFactory createConnectionFactory(boolean multiPartitionMode) {
    return JdbcConnection.patternBasedFactory(createUrlPattern(multiPartitionMode),
                                              ...
                                              JdbcConfiguration.PORT
                                                .withDefault(SqlServerConnectorConfig.PORT.defaultValueAsString()));
  }
  ...
  public Lsn getNthTransactionLsnFromBeginning(String databaseName, int maxOffset) throws SQLException {
    return prepareQueryAndMap(replaceDatabaseNamePlaceholder(GET_NTH_TRANSACTION_LSN_FROM_BEGINNING, databaseName),
                              statement -> {
  ...
  public Lsn getNthTransactionLsnFromLast(String databaseName, Lsn lastLsn, int maxOffset) throws SQLException {
    return prepareQueryAndMap(replaceDatabaseNamePlaceholder(GET_NTH_TRANSACTION_LSN_FROM_LAST, databaseName),
                              statement -> {
      statement.setInt(1, maxOffset);
  ...
  private Lsn getFromLsn(String databaseName, SqlServerChangeTable changeTable, Lsn intervalFromLsn)
    throws SQLException {
    Lsn fromLsn = changeTable.getStartLsn().compareTo(intervalFromLsn) > 0 ?
  ...
  public List<SqlServerChangeTable> getNewChangeTables(String databaseName, Lsn fromLsn, Lsn toLsn)
    throws SQLException {
