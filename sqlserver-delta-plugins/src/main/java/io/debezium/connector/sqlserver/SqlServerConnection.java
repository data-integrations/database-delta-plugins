...
public class SqlServerConnection extends JdbcConnection {
   ...
   * <p>
   * If two or more capture instances with the same start LSN are available for a given source
   * table, only the newest one will be returned.
   * <p>
   * We use a query instead of {@code sys.sp_cdc_help_change_data_capture} because: 1. The stored
   * procedure doesn't allow filtering capture instances by start LSN. 2. There is no way to use the
   * result returned by a stored procedure in a query.
   */
  ...
  private static final String URL_PATTERN =
      "jdbc:sqlserver://${" + JdbcConfiguration.HOSTNAME + "}:${" + JdbcConfiguration.PORT + "}";
  ...
  public SqlServerConnection(JdbcConfiguration config, SourceTimestampMode sourceTimestampMode,
    ...
    /** TODO: We were using a public ConnectionFactory object, and setting it in
     * SQLServerEventReader.java to be used here.
     * We can probably implement an overloaded createConnectionFactory method
     * and use it to set the connection factory in SQLServerEventReader.java
     */
    super(config, createConnectionFactory(multiPartitionMode), classLoaderSupplier,
        OPENING_QUOTING_CHARACTER, CLOSING_QUOTING_CHARACTER);

    if (config().hasKey(SERVER_TIMEZONE_PROP_NAME)) {
      LOGGER.warn("The '{}' option is deprecated and is not taken into account",
  ...
  public Lsn getMaxLsn(String databaseName) throws SQLException {
    return queryAndMap(replaceDatabaseNamePlaceholder(GET_MAX_LSN, databaseName),
  ...
  /**
   * @return the log sequence number of the most recent transaction that isn't further than
   * {@code maxOffset} from the beginning.
   */
  ...
  /**
   * @return the log sequence number of the most recent transaction that isn't further than
   * {@code maxOffset} from {@code lastLsn}.
   */
  ...
  public Lsn getMaxTransactionLsn(String databaseName) throws SQLException {
    return queryAndMap(replaceDatabaseNamePlaceholder(GET_MAX_TRANSACTION_LSN, databaseName),
        singleResultMapper(rs -> {
          final Lsn ret = Lsn.valueOf(rs.getBytes(1));
   ...
   * @throws SQLException
   */
  public void getChangesForTables(String databaseName, SqlServerChangeTable[] changeTables,
      Lsn intervalFromLsn,
  ...
  private Lsn getFromLsn(String databaseName, SqlServerChangeTable changeTable, Lsn intervalFromLsn)
      throws SQLException {
    Lsn fromLsn =
        changeTable.getStartLsn().compareTo(intervalFromLsn) > 0 ? changeTable.getStartLsn()
            : intervalFromLsn;
    return fromLsn.getBinary() != null ? fromLsn
  ...
  public Lsn incrementLsn(String databaseName, Lsn lsn) throws SQLException {
    return prepareQueryAndMap(replaceDatabaseNamePlaceholder(INCREMENT_LSN, databaseName),
        statement -> {
  ...
  public Table getTableSchemaFromTable(String databaseName, SqlServerChangeTable changeTable)
    ...
    final List<String> pkColumnNames = readPrimaryKeyOrUniqueIndexNames(metadata,
   ...
   * <p>
   * Although SQL Server supports case-insensitive collations, the connector uses the database name
   * to build the produced records' source info and, subsequently, the keys of its committed offset
   * messages. This value must remain the same during the lifetime of the connector regardless of
   * the case used in the connector configuration.
   */
