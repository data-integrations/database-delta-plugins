...
public class MySqlJdbcContext implements AutoCloseable {
  ...
  /**
   * ===================== This is a diff from the original file =========================== We
   * intentionally make the 'MYSQL_CONNECTION_URL' a public static variable, so that we can set the
   * value in app source.
   */
  public static final String MYSQL_CONNECTION_URL =
      "jdbc:mysql://${hostname}:${port}/?useInformationSchema=true&" +
  ...
  public MySqlJdbcContext(MySqlConnectorConfig config) {
    ...
    if (useSSL) {
      ...
      if (sslTrustStorePassword() != null) {
        jdbcConfigBuilder.with("trustCertificateKeyStorePassword",
      ...
      if (sslKeyStorePassword() != null) {
        jdbcConfigBuilder.with("clientCertificateKeyStorePassword",
   ...
   *
   * @return {@code false} if the server's {@code gtid_mode} is set and is {@code OFF}, or
   * {@code true} otherwise
   */
  ...
  public GtidSet purgedGtidSet() {
    ...
    try {
      ...
      throw new ConnectException(
          "Unexpected error while connecting to MySQL and looking at gtid_purged variable: ", e);
    }
  ...
  protected Map<String, DatabaseLocales> readDatabaseCollations() {
    ...
    try {
      return jdbc.connect().queryAndMap(
    ...
    if (value != null) {
      ...
      if (existingValue == null) {
        ...
        if (!existingValue.equalsIgnoreCase(value)) {
          ...
          String msg = "System or JVM property '" + property
          ...
          if (showValueInError) {
            msg = "System or JVM property '" + property + "' is already defined as " + existingValue
                + ", but the configuration property '" + field.name()
                + "' defines a different value '" + value + "'";
          }
  ...
  public String getSessionVariableForSslVersion() {
    ...
    Map<String, String> sessionVariables = querySystemVariables(
