...
public class MySqlConnectorConfig extends HistorizedRelationalDatabaseConnectorConfig {
  ...
  public static final Field ON_CONNECT_STATEMENTS = Field.create("database.initial.statements")
    ...
    .withDescription(
      "A semicolon separated list of SQL statements to be executed when a JDBC connection " +
  ...
  public static final Field SSL_KEYSTORE = Field.create("database.ssl.keystore")
    ...
    .withDescription("The location of the key store file. "
                       + "This is optional and can be used for two-way authentication between the client and the " +
                       "MySQL Server.");
  ...
  public static final Field GTID_SOURCE_FILTER_DML_EVENTS = Field.create("gtid.source.filter.dml.events")
    ...
    .withDescription(
      "If set to true, we will only produce DML events into Kafka for transactions that were written on mysql " +
        "servers " +
        "with UUIDs matching the filters defined by the gtid.source.includes or gtid.source.excludes configuration " +
  ...
  public static final Field SNAPSHOT_MODE = Field.create("snapshot.mode")
    ...
    .withDescription("The criteria for running a snapshot upon startup of the connector. "
                       + "Options include: "
                       + "'when_needed' to specify that the connector run a snapshot upon startup whenever it deems " +
                       "it necessary; "
  ...
  public static final Field SNAPSHOT_LOCKING_MODE = Field.create("snapshot.locking.mode")
    ...
    .withDescription("Controls how long the connector holds onto the global read lock while it is performing a " +
                       ...
                       + "which means the connector holds the global read lock (and thus prevents any updates) " +
                       "for just the initial portion of the snapshot "
  ...
  public static final Field SNAPSHOT_NEW_TABLES = Field.create("snapshot.new.tables")
    ...
    .withWidth(Width.SHORT)
    .withImportance(Importance.LOW)
    .withDescription("BETA FEATURE: On connector restart, the connector will check if there have been any new " +
                       "tables added to the configuration, "
  ...
  public static final Field READ_ONLY_CONNECTION = Field.create("read.only")
    ...
    .withWidth(Width.SHORT)
    .withImportance(Importance.LOW)
    .withDescription("Switched connector to use alternative methods to deliver signals to Debezium instead of " +
                       "writing to signaling table");
  /**
  ...
  protected static final Set<String> BUILT_IN_DB_NAMES = Collect.unmodifiableSet("mysql",
                                                                                 "performance_schema",
   ...
   * latest executed position (default). When set to 'earliest' the connector will start reading new GTID channels from
   * the first available position.
   * This is useful when in active-passive mysql setup during failover new GTID channel starts receiving writes,
   * see DBZ-923.
   * <p>
   * Defaults to latest.
   */
  ...
  private static final ConfigDefinition CONFIG_DEFINITION = HistorizedRelationalDatabaseConnectorConfig
    .CONFIG_DEFINITION.edit()
    .name("MySQL")
    .excluding(
      SCHEMA_WHITELIST,
      SCHEMA_INCLUDE_LIST,
  ...
  public MySqlConnectorConfig(Configuration config) {
    ...
    this.config = config;
    this.legacy = MySqlConnector.isLegacy(
      config.getString(io.debezium.connector.mysql.MySqlConnector.IMPLEMENTATION_PROP));
    this.temporalPrecisionMode = TemporalPrecisionMode.parse(config.getString(TIME_PRECISION_MODE));
    this.snapshotMode = SnapshotMode.parse(config.getString(SNAPSHOT_MODE), SNAPSHOT_MODE.defaultValueAsString());
    this.snapshotLockingMode = SnapshotLockingMode.parse(
      config.getString(SNAPSHOT_LOCKING_MODE), SNAPSHOT_LOCKING_MODE.defaultValueAsString());
    ...
    this.gitIdNewChannelPosition = GtidNewChannelPosition.parse(
      gitIdNewChannelPosition, MySqlConnectorConfig.GTID_NEW_CHANNEL_POSITION.defaultValueAsString());

    final String snapshotNewTables = config.getString(MySqlConnectorConfig.SNAPSHOT_NEW_TABLES);
    this.snapshotNewTables = SnapshotNewTables.parse(
      snapshotNewTables, MySqlConnectorConfig.SNAPSHOT_NEW_TABLES.defaultValueAsString());
    ...
    this.inconsistentSchemaFailureHandlingMode = EventProcessingFailureHandlingMode
      .parse(inconsistentSchemaFailureHandlingMode);
  ...
  private static int validateSnapshotLockingMode(Configuration config, Field field, ValidationOutput problems) {
    ...
    if (config.hasKey(SNAPSHOT_LOCKING_MODE.name())) {
      ...
      if (lockingModeValue == null) {
        problems
          .accept(SNAPSHOT_LOCKING_MODE, lockingModeValue, "Must be a valid snapshot.locking.mode value");
        return 1;
      }
    }
  ...
  private static int validateTimePrecisionMode(Configuration config, Field field, ValidationOutput problems) {
    if (config.hasKey(TIME_PRECISION_MODE.name())) {
      ...
      if (TemporalPrecisionMode.ADAPTIVE.getValue().equals(timePrecisionMode)) {
        // this is a problem
        problems.accept(TIME_PRECISION_MODE, timePrecisionMode, "The 'adaptive' time.precision.mode " +
  ...
  protected SourceInfoStructMaker<? extends AbstractSourceInfo> getSourceInfoStructMaker(Version version) {
    switch (version) {
      case V1:
        return legacy ? new io.debezium.connector.mysql.legacy
          .LegacyV1MySqlSourceInfoStructMaker(Module.name(), Module.version(), this)
          : new LegacyV1MySqlSourceInfoStructMaker(Module.name(), Module.version(), this);
      default:
        return legacy ? new io.debezium.connector.mysql.legacy
          .MySqlSourceInfoStructMaker(Module.name(), Module.version(), this)
  ...
  public enum SnapshotMode implements EnumeratedValue {
    ...
    /**
     * Perform a snapshot of only the database schemas (without data) and then begin reading the binlog at the
     * current binlog position.
     * This can be used for recovery only if the connector has existing offsets and the database.history.kafka.topic
     * does not exist (deleted).
     * This recovery option should be used with care as it assumes there have been no schema changes since the
     * connector last stopped,
  ...
  /**
   * Enum for Snapshot new tables : off or execute in parallel
   */
  ...
  public enum SnapshotLockingMode implements EnumeratedValue {
    ...
    /**
     * The connector holds the global read lock for just the initial portion of the snapshot while the connector
     * reads the database
     * schemas and other metadata. The remaining work in a snapshot involves selecting all rows from each table,
     * and this can be done
     * in a consistent fashion using the REPEATABLE READ transaction even when the global read lock is no longer
     * held and while other
    ...
    /**
     * The connector holds a (Percona-specific) backup lock for just the initial portion of the snapshot while
     * the connector
     * reads the database schemas and other metadata. This lock will only block DDL and DML on non-transactional
     * tables
     * (MyISAM etc.). The remaining work in a snapshot involves selecting all rows from each table, and this can
     * be done in a
     * consistent fashion using the REPEATABLE READ transaction even when the global read lock is no longer held
     * and while other
     * MySQL clients are updating the database.
     */
