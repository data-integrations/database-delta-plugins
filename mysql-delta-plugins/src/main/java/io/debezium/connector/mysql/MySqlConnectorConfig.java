...
public class MySqlConnectorConfig extends HistorizedRelationalDatabaseConnectorConfig {
   ...
   * provides global locking. So to bypass this limitation it is necessary to provide a backdoor to
   * connector to disable it on its own.
   */
  ...
  public static enum SnapshotMode implements EnumeratedValue {
    ...
    private SnapshotMode(String value, boolean includeSchema, boolean includeData,
        boolean shouldStream, boolean shouldSnapshotOnSchemaError,
  ...
  public static final Field BUFFER_SIZE_FOR_BINLOG_READER = Field.create("binlog.buffer.size")
      ...
      .withImportance(Importance.MEDIUM)
      .withDescription(
          "The size of a look-ahead buffer used by the  binlog reader to decide whether "
  ...
  public static final Field SNAPSHOT_LOCKING_MODE = Field.create("snapshot.locking.mode")
      ...
      .withImportance(Importance.LOW)
      .withDescription(
          "Controls how long the connector holds onto the global read lock while it is performing a snapshot. The default is 'minimal', "
  ...
  public static final Field SNAPSHOT_NEW_TABLES = Field.create("snapshot.new.tables")
      ...
      .withImportance(Importance.LOW)
      .withDescription(
          "BETA FEATURE: On connector restart, the connector will check if there have been any new tables added to the configuration, "
  ...
  public static final Field EVENT_DESERIALIZATION_FAILURE_HANDLING_MODE = Field.create(
  ...
  public MySqlConnectorConfig(Configuration config) {
    ...
    this.snapshotMode = SnapshotMode.parse(config.getString(SNAPSHOT_MODE),
        SNAPSHOT_MODE.defaultValueAsString());
    ...
    final String gitIdNewChannelPosition = config.getString(
        MySqlConnectorConfig.GTID_NEW_CHANNEL_POSITION);
    this.gitIdNewChannelPosition = GtidNewChannelPosition.parse(gitIdNewChannelPosition,
    ...
    this.inconsistentSchemaFailureHandlingMode = EventProcessingFailureHandlingMode.parse(
    ...
    this.sourceInfoStructMaker = getSourceInfoStructMaker(
        Version.parse(config.getString(SOURCE_STRUCT_MAKER_VERSION)));
  }
  ...
  private static int validateEventDeserializationFailureHandlingModeNotSet(Configuration config,
      Field field, ValidationOutput problems) {
  ...
  private static int validateInconsistentSchemaHandlingModeNotIgnore(Configuration config,
      Field field, ValidationOutput problems) {
  ...
  private static int validateTimePrecisionMode(Configuration config, Field field, ValidationOutput problems) {
    if (config.hasKey(TIME_PRECISION_MODE.name())) {
      ...
      if (TemporalPrecisionMode.ADAPTIVE.getValue().equals(timePrecisionMode)) {
        // this is a problem
        problems.accept(TIME_PRECISION_MODE, timePrecisionMode,
            "The 'adaptive' time.precision.mode is no longer supported");
        return 1;
      }
    }
  ...
  protected SourceInfoStructMaker<? extends AbstractSourceInfo> getSourceInfoStructMaker(Version version) {
    switch (version) {
      case V1:
        return legacy ? new io.debezium.connector.mysql.legacy.LegacyV1MySqlSourceInfoStructMaker(
            Module.name(), Module.version(), this)
            : new LegacyV1MySqlSourceInfoStructMaker(Module.name(), Module.version(), this);
      default:
        return legacy ? new io.debezium.connector.mysql.legacy.MySqlSourceInfoStructMaker(
            Module.name(), Module.version(), this)
  ...
  /**
   * Get the predicate function that will return {@code true} if a GTID source is to be included, or
   * {@code false} if a GTID source is to be excluded.
   *
