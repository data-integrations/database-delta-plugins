 ...
 * deserialize} these as {@link java.sql.Timestamp} values that have no timezone and, therefore, are presumed to be in
 * UTC.
 * When the column is properly marked with a {@link Types#TIMESTAMP_WITH_TIMEZONE} type, the converters will need to
 * convert
 * that {@link java.sql.Timestamp} value into an {@link OffsetDateTime} using the default time zone, which always is
 * UTC.
 *
...
public class MySqlValueConverters extends JdbcValueConverters {
  ...
  public MySqlValueConverters(DecimalMode decimalMode, TemporalPrecisionMode temporalPrecisionMode,
    ...
    this(decimalMode, temporalPrecisionMode, bigIntUnsignedMode, binaryMode, x -> x,
   ...
   */
  public Charset charsetFor(Column column) {
...
//    encoding = MySqlConnection.getJavaEncodingForMysqlCharSet(mySqlCharsetName);
    if (encoding == null) {
  ...
  protected Object convertPoint(Column column, Field fieldDefn, Object data) {
    ...
    return convertValue(column, fieldDefn, data, io.debezium.data.geometry.Geometry.createValue(fieldDefn.schema(),
                                                                                                empty.getWkb(),
                                                                                                empty.getSrid()),
                        (r) -> {
                          if (data instanceof byte[]) {
                            // The binlog utility sends a byte array for any Geometry type, we will use our own
                            // binaryParse to parse the
                            // byte to WKB, hence
  ...
  protected Object convertGeometry(Column column, Field fieldDefn, Object data) {
    ...
    return convertValue(column, fieldDefn, data, io.debezium.data.geometry.Geometry
      ...
      if (data instanceof byte[]) {
        // The binlog utility sends a byte array for any Geometry type, we will use our own binaryParse to parse
        // the byte to WKB, hence
        // to the suitable class
        if (data instanceof byte[]) {
          // The binlog utility sends a byte array for any Geometry type, we will use our own binaryParse to parse
          // the byte to WKB, hence
  ...
  protected Object convertUnsignedBigint(Column column, Field fieldDefn, Object data) {
    return convertValue(column, fieldDefn, data, 0L, (r) -> {
      if (data instanceof BigDecimal) {
        ...
        r.deliver(MySqlUnsignedIntegerConverter.convertUnsignedBigint(new BigDecimal(data.toString())));
      } else if (data instanceof String) {
  ...
  /**
   * A functional interface for a handler to parse errors
   */
  @FunctionalInterface
